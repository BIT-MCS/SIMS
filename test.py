...
def test():
    ...
    seq_model.load_state_dict(torch.load('./log/dnc_fri_new_pytorch/2020/05-07/21-28-18/model.pth'))
    seq_model.eval()
    with torch.no_grad():
        st = time.time()
        MAP, res_list = evaluate(seq_model, data)
        # MAP, log_str = test_evaluate(seq_model, data)
        # if MAP > 0.40 and MAP < 0.42036:
        print(MAP)
        ed = time.time() - st
